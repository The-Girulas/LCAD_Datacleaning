...
fn infer_column_types(
    ...
    for (line_num, record_result) in csv_reader.records().enumerate() {
        let record = match record_result {
            Ok(r) => r,
            Err(err) => {
                // Log error for this specific line but try to continue inference if possible
                eprintln!(
                    "Avertissement: Erreur de lecture CSV durant l'inférence à la ligne {}: {}. Ligne ignorée.",
                    line_num + 1, // records() is 0-indexed for lines
                    err
                );
...
fn main() -> anyhow::Result<()> {
    ...
    dbg!(&inferred_column_types);

    let input_file = File::open(&args.file)?;
    let initial_reader = BufReader::new(input_file);

    let encoding_obj = match args.encoding.to_lowercase().as_str() { // Renamed to encoding_obj to avoid conflict
        "utf-8" => UTF_8,
        "windows-1252" => WINDOWS_1252,
        "iso-8859-1" => ISO_8859_1,
        other => {
    ...
    let transcoded_reader = encoding_rs_io::DecodeReaderBytesBuilder::new()
        .encoding(Some(encoding_obj)) // Use encoding_obj
        .build(initial_reader);
    ...
    for record_result in csv_reader.records() {
        let record = match record_result {
            Ok(r) => r,
            Err(e) => {
                // Add newline if progress was shown, to not overwrite it
                if progress_shown { eprintln!(); progress_shown = false; }
                eprintln!("Erreur de parsing CSV sur la ligne {}: {}", count + 1, e);
                bad +=1;
                let error_line = format!("#ERROR (parsing error: {})", e);
                writeln!(writer, "{}", error_line)?;
                count += 1; // Increment count for processed (even if errored) lines
                if let Some(max_lines) = args.max {
                    if count >= max_lines {
                        // Add newline if progress was shown
        ...
        if count % 100_000 == 0 {
            ...
            std::io::stdout().flush()?; // Use ? for error handling
            progress_shown = true;
        }

        if let Some(max_lines) = args.max {
            if count >= max_lines {
                if progress_shown { println!(); } // Add newline if progress was shown
                println!("Limite de {max_lines} lignes atteinte.");
                break;
            }
        }
    ...
    if progress_shown { println!(); } // Add final newline if progress was shown
